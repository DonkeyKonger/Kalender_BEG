...
test("Baustellenauswertung zeigt die realisierte Monatsleistung mit positivem und negativem Ergebnis", () => {
  assert.match(component, /Aufmaßstunden[\s\S]*?Zusatzaufträge[\s\S]*?Realisierte Ist-Stunden[\s\S]*?Ergebnis/s);
...
test("die Erklärung macht Realisierungsereignis, offene Werte und Formel nachvollziehbar", () => {
  ...
  assert.match(component, /Ergebnis = Aufmaßstunden \+ Zusatzauftragsstunden − realisierte Ist-Stunden/);
});
