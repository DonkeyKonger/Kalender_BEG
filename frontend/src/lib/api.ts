...
export const api = {
  ...
  async personToolMaterialItems(personId: number): Promise<PersonToolMaterialItem[]> {
    return request<PersonToolMaterialItem[]>(`/persons/${personId}/tool-material-items`);
  },
  ...
  async toolMaterialItemsPage(params: ToolMaterialListParams = {}): Promise<ToolMaterialPage> {
    const search = buildToolMaterialSearchParams(params);
    const suffix = search.toString() ? `?${search.toString()}` : "";
    return request<ToolMaterialPage>(`/admin/tool-material-items/page${suffix}`);
  },

  async toolMaterialItem(itemId: number): Promise<ToolMaterialItem> {
  ...
  async toolMaterialFilterOptionsForColumn(column: string): Promise<ToolMaterialFilterOption[]> {
    return request<ToolMaterialFilterOption[]>(
      `/admin/tool-material-items/filter-options/${encodeURIComponent(column)}`,
    );
  },

  async toolMaterialResponsibility(): Promise<ToolMaterialResponsibility> {
    return request<ToolMaterialResponsibility>("/admin/tool-material-items/responsibility");
  },

  async toolMaterialResponsibleUserOptions(): Promise<ToolResponsibleUser[]> {
    return request<ToolResponsibleUser[]>("/admin/tool-material-items/responsibility/options");
  },

  async updateToolMaterialResponsibility(
    toolResponsibleUserId: number | null,
  ): Promise<ToolMaterialResponsibility> {
    return request<ToolMaterialResponsibility>("/admin/tool-material-items/responsibility", {
      method: "PUT",
      body: JSON.stringify({ tool_responsible_user_id: toolResponsibleUserId }),
    });
  },
  ...
  async timeEntryDayStatus(params: { personId: number; workDate: string }): Promise<PersonWorkDay> {
    const search = new URLSearchParams({
      person_id: String(params.personId),
      work_date: params.workDate,
    });
    return request<PersonWorkDay>(`/time-entries/day-status?${search.toString()}`, { cache: "no-store" });
  },

  async setTimeEntryDayOvernightStatus(params: {
    personId: number;
    workDate: string;
    overnightStatus: OvernightStatus;
  }): Promise<PersonWorkDay> {
    const search = new URLSearchParams({
      person_id: String(params.personId),
      work_date: params.workDate,
    });
    return request<PersonWorkDay>(`/time-entries/day-status?${search.toString()}`, {
      method: "PATCH",
      body: JSON.stringify({ overnight_status: params.overnightStatus }),
    });
  },
  ...
  async reopenPayrollPersonMonth(params: { year: number; month: number; personId: number; reason: string }): Promise<PayrollMonthPeriod> {
    return request<PayrollMonthPeriod>(`/payroll-months/${params.year}/${params.month}/people/${params.personId}/reopen`, {
      method: "POST",
      body: JSON.stringify({ reason: params.reason }),
    });
  },
  ...
  async siteMeasurementWorkers(siteId: number): Promise<MeasurementWorkerOption[]> {
    return request<MeasurementWorkerOption[]>(`/sites/${siteId}/measurement-workers`);
  },

  async createOfficeMeasurementBatch(
    siteId: number,
  ...
  async promoteSiteMeasurementBatchStatus(
    siteId: number,
    batchId: number,
    status: MeasurementManualStatus,
  ): Promise<MobileMeasurementBatch> {
    return request<MobileMeasurementBatch>(`/sites/${siteId}/measurement-batches/${batchId}/status`, {
      method: "PATCH",
  ...
  async promoteSiteExtraWorkTicketStatus(
    siteId: number,
    ticketId: number,
    status: ExtraWorkManualStatus,
  ): Promise<MobileExtraWorkTicket> {
    return request<MobileExtraWorkTicket>(`/sites/${siteId}/extra-work-tickets/${ticketId}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    });
  },
  ...
  async myAssignmentSites(): Promise<MobileAssignmentSitesResponse> {
    return request<MobileAssignmentSitesResponse>("/me/assignment-sites", {
      cache: "no-store",
    });
  },

  async myAssignmentSiteHistory(siteId: number): Promise<MobileAssignmentSiteHistoryResponse> {
    return request<MobileAssignmentSiteHistoryResponse>(`/me/assignment-sites/${siteId}/history`, {
      cache: "no-store",
    });
  },
  ...
  async myPersonalFileAbsences(params: {
    absenceType: MobilePersonalFileAbsenceType;
    year: number;
  }): Promise<MobilePersonalFileAbsenceResponse> {
    const search = new URLSearchParams({
      absence_type: params.absenceType,
      year: String(params.year),
    });
    return request<MobilePersonalFileAbsenceResponse>(`/me/personal-file/absences?${search.toString()}`, {
      cache: "no-store",
    });
  },
  ...
  async updateMobileExtraWorkTicketDetails(
    assignmentId: number,
    ticketId: number,
    payload: MobileExtraWorkTicketDetailsUpdate,
  ): Promise<MobileExtraWorkTicket> {
    return request<MobileExtraWorkTicket>(`/me/assignments/${assignmentId}/extra-work-tickets/${ticketId}/details`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
  },
