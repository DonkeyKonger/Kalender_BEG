...
export function getMeasurementReviewSteps(batch: Pick<MobileMeasurementBatch, "status" | "submitted_at" | "customer_signed_at" | "customer_signature_name">) {
  ...
  const evidence = [Boolean(batch.submitted_at) || submitted, reviewed, signed, completed];
