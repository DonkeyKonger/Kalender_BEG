...
class DashboardNoteService:
    ...
    def list_notes(self, completed: bool | None = None) -> list[DashboardNote]:
        statement = (
            select(DashboardNote)
            .options(selectinload(DashboardNote.site), selectinload(DashboardNote.employee))
            .where(DashboardNote.deleted_at.is_(None))
        )
    ...
    def create_note(self, payload: DashboardNoteCreate, user_id: int | None) -> DashboardNote:
        ...
        return self._get_note(note.id) or note

    def update_note(self, note_id: int, payload: DashboardNoteUpdate) -> DashboardNote:
    ...
    def delete_note(self, note_id: int, user_id: int | None) -> None:
        note = self._get_note(note_id)
        if note is None:
            raise HTTPException(status.HTTP_404_NOT_FOUND, "Notiz nicht gefunden.")
    ...
    def _get_note(self, note_id: int) -> DashboardNote | None:
        return self.db.scalar(
            select(DashboardNote)
            .options(selectinload(DashboardNote.site), selectinload(DashboardNote.employee))
            .where(DashboardNote.id == note_id, DashboardNote.deleted_at.is_(None))
        )
